...
const Restaurant = (props) => {
  ...
  const [mobile, setMobile] = useState("7981030872");
  const [address, setAddress] = useState("Arunodaya Nagar, Navghar Road, Mulund (east), Delhi, 110019");


  // apiRoutes.get("/api/restaurants-list-by-restaurant-id/:query_name",restaurant.getRestaurantListByRestaurantId)
    let getRestaurantDetails = async () => {
    let url =  `http://localhost:2907/api/restaurants-list-by-restaurant-id/` + id;
  ...
  let getMenuItems = async () => {
    let url = `http://localhost:2907/api/menu-items-list-by-restaurant-id/` + id;
    let { data } = await axios.get(url);
    setTotal(0);
    setMenuItemList(data.menulist);
  ...
  let makePayment = async () => {
    ...
    let url = "http://localhost:2907/api/gen-order-details";
    ...
    var options = {
      ...
      handler: async (response) => {
        ...
        let url = "http://localhost:2907/api/verify-payment";
